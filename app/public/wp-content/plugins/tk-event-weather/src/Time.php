...
class Time {
	 ...
	 *
	 * @see Time::set_timezone_and_source_from_shortcode_args()
	...
	public static function set_timezone_from_api( $timezone_from_api = '' ) {
		if (
			...
			&& 'api' == Shortcode::$timezone_source // should always be true
		) {
			...
			if ( in_array( $timezone, timezone_identifiers_list() ) ) {
				...
				Functions::invalid_shortcode_message( 'The Timezone could not be set via the Dark Sky API. Please enable debug and investigate further' );
			}
		}
	}
	...
	public static function valid_iso_8601_date_time( $input, $return_format = '' ) {
		$result = Functions::remove_all_whitespace( $input );
	...
	public static function timestamp_is_during_today( $timestamp ) {
		...
		$now            = time();
		$today_midnight = self::get_a_days_min_max_timestamp( $now, Shortcode::$timezone );
		$today_end      = self::get_a_days_min_max_timestamp( $now, Shortcode::$timezone, true );
