...
if ( !class_exists( 'WTBP_WPTAO_Settings_API' ) ):

	class WTBP_WPTAO_Settings_API {
		...
		public function settings_init() {
			...
			foreach ( $this->settings_fields as $section => $field ) {
				foreach ( $field as $option ) {
					...
					$args = array(
						'id'				 => $option[ 'name' ],
						'label_for'			 => $args[ 'label_for' ] = "wtbpsettings[[{$option[ 'name' ]}]",
		...
		function callback_text( $args ) {
			...
			$class	 = !empty( $args[ 'class' ] ) ? sanitize_title( $args[ 'class' ] ) : '';
		...
		function callback_checkbox( $args ) {
			...
			$html = '<fieldset>';
			$html .= sprintf( '<label for="wpuf-%1$s[%2$s]">', $this->name, $args[ 'id' ] );
			$html .= sprintf( '<input type="hidden" name="%1$s[%2$s]" value="off" />', $this->name, $args[ 'id' ] );
			$html .= sprintf( '<input type="checkbox" class="checkbox %4$s" id="wpuf-%1$s[%2$s]" name="%1$s[%2$s]" value="on" %3$s />', $this->name, $args[ 'id' ], checked( $value, 'on', false ), $class );
		...
		function callback_multicheck( $args ) {
			...
			foreach ( $args[ 'options' ] as $key => $label ) {
				...
				$html .= sprintf( '<input type="checkbox" class="checkbox %5$s" id="%1$s[%2$s][%3$s]" name="%1$s[%2$s][%3$s]" value="%3$s" %4$s />', $this->name, $args[ 'id' ], $key, checked( $checked, $key, false ), $class );
		...
		function callback_radio( $args ) {
			...
			foreach ( $args[ 'options' ] as $key => $label ) {
				...
				$html .= sprintf( '<input type="radio" class="radio %6$s" id="%1$s%2$s[%3$s][%4$s]" name="%2$s[%3$s]" value="%4$s" %5$s />', $this->prefix, $this->name, $args[ 'id' ], $key, checked( $value, $key, false ), $class );
				$html .= sprintf( '%1$s</label><br>', $label );
			}
			$html .= $this->get_field_description( $args );
			$html .= '</fieldset>';
		...
		function callback_select( $args ) {
			...
			$html = sprintf( '<select class="%1$s %4$s" name="%2$s[%3$s]" id="%2$s[%3$s]">', $size, $this->name, $args[ 'id' ], $class );
			foreach ( $args[ 'options' ] as $key => $label ) {
		...
		function callback_textarea( $args ) {
			...
			$value	 = wp_kses_post( $this->get_option( $args[ 'id' ], $args[ 'std' ] ) );
			$size	 = isset( $args[ 'size' ] ) && !is_null( $args[ 'size' ] ) ? $args[ 'size' ] : 'regular';
			$class	 = !empty( $args[ 'class' ] ) ? sanitize_title( $args[ 'class' ] ) : '';

			$html = sprintf( '<textarea rows="5" cols="55" class="%1$s-text %4$s" id="%2$s[%3$s]" name="%2$s[%3$s]">%4$s</textarea>', $size, $this->name, $args[ 'id' ], $value, $class );
			$html .= $this->get_field_description( $args );
		...
		function callback_wysiwyg( $args ) {
			...
			if ( !empty( $args[ 'class' ] ) ) {
				$editor_settings[ 'editor_class' ] = sanitize_title( $args[ 'class' ] );
			}
		...
		function callback_file( $args ) {
			...
			$html = sprintf( '<input type="text" class="%1$s-text %2$surl %6$s" id="%3$s[%4$s]" name="%3$s[%4$s]" value="%5$s"/>', $size, $this->prefix, $this->name, $args[ 'id' ], $value, $class );
		...
		function callback_password( $args ) {
			...
			$html = sprintf( '<input type="password" class="%1$s-text %5$s" id="%2$s[%3$s]" name="%2$s[%3$s]" value="%4$s"/>', $size, $this->name, $args[ 'id' ], $value, $class );
			$html .= $this->get_field_description( $args );
		...
		function callback_color( $args ) {
			...
			$html = sprintf( '<input type="text" class="%1$s-text wp-color-picker-field %6$s" id="%2$s[%3$s]" name="%2$s[%3$s]" value="%4$s" data-default-color="%5$s" />', $size, $this->name, $args[ 'id' ], $value, $args[ 'std' ], $class );
			$html .= $this->get_field_description( $args );
